...
import com.keimons.nutshell.disruptor.support.DirectExecutor;
import com.keimons.nutshell.disruptor.support.QueueTrackExecutor;
 ...
 * 任务执行器
 * <p>
 * 系统允许定义最多128个任务执行器。每个执行器需要实现{@link TrackExecutor}接口。
 *
 * @author houyn[monkey@keimons.com]
 * @version 1.0
 * @see DirectExecutor 即时执行器（无队列）
 * @see SharedQueueExecutor 共享队列执行器
 * @see QueueTrackExecutor 哈希队列执行器
 * @since 11
 **/
public class ExecutorManager {
	 ...
	 *
	 * @see DirectExecutor 无队列任务执行器
	 * @see QueueTrackExecutor 哈希队列任务执行器
	 * @see SharedQueueExecutor 共享队列任务执行器
	 */
	private static final TrackExecutor[] strategies = new TrackExecutor[128];
	...
	public static <T extends TrackExecutor> T getExecutorStrategy(
	...
	public static synchronized void registerExecutorStrategy(
			@Range(from = 0, to = 127) int executorIndex,
			@NotNull TrackExecutor strategy) {
